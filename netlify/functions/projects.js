...
async function GetList(userId) {
  ...
  let list = await s3api.docList(`users/${userId}/projects`);
  return {
    statusCode: 200,
...
async function ReadOne(userId, projId) {
  ...
  let doc = await s3api.docGet(`users/${userId}/projects/${projId}.json`);
  return {
...
async function CreateOne(userId, json) {
  ...
  let result = await s3api.docPut(`users/${userId}/projects/${projId}.json`, newDoc);
  return {
    statusCode: 200,
    headers: { 'Content-Type': 'application/json'},
...
exports.handler = async (event, /* context */ ) => {
  try {
    ...
    let parts = chopURL(event.path, 'projects');
    let projId = '';
    if (parts[1]) {
      projId = parts[1].trim();
    }
